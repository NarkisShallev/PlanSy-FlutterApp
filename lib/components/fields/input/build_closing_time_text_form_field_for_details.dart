...
class BuildClosingTimeFormFieldForDetails extends StatefulWidget {
  final MaskTextInputFormatter formatter = MaskTextInputFormatter(mask: "##:##");
  final Function onSaved;
  final BuildContext context;
  final Function setClosingTime;
  ...
  @override
  _BuildClosingTimeFormFieldForDetailsState createState() =>
      _BuildClosingTimeFormFieldForDetailsState();
}

class _BuildClosingTimeFormFieldForDetailsState extends State<BuildClosingTimeFormFieldForDetails> {
  TextEditingController _closingTimeController = TextEditingController();
  TimeOfDay closingTime = TimeOfDay(hour: 0, minute: 0);
  TimeOfDay time;

  @override
  void initState() {
    super.initState();
    if (_closingTimeController.text == "") {
      _closingTimeController.text = widget.initialValue;
    }
  }
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      controller: widget.isEnabled ? _closingTimeController : null,
      ...
      validator: (value) {
        if (value.isEmpty) {
          return kClosingTimeNullError;
        }
        if (value.isNotEmpty && !isTimeValid(value)) {
          return kInvalidClosingTimeError;
        }
        return null;
      },
      decoration: InputDecoration(
        labelText: widget.isUpdate ? "" : "* Closing time",
        ...
        suffixIcon: CustomSuffixIconButton(
          ...
          onPressed: () async {
            ...
            updateVars(widget.setClosingTime);
          },
        ),
      ),
    );
